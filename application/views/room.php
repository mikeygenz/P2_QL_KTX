<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://unpkg.com/react@18/umd/react.production.min.js" crossorigin></script>
    <script src="https://unpkg.com/react-dom@18/umd/react-dom.production.min.js" crossorigin></script>
    <link href="../css/home.css" rel="stylesheet">
    <title>KTX</title>
</head>
<body>
    <div >
        <?php include_once("header.php") ?>
        <?php include_once("top_menu.php") ?>
    </div>
    <div class="main-content">
        <div class="row">
            <?php include_once("navbar.php") ?>
            <div class="col-10">
                <div class="view_room" >
                    <table>
                        <tr>                        
                            <th style="width:15%">Tên phòng</th>
                            <th style="width:15%">Số giường</th>
                            <th style="width:15%">Giá phòng</th>
                            <th>Cở sở vật chất</th>
                            <th style="width:15%">Tùy chọn</th>
                        </tr>
                            <?php 
                                if(isset($_POST["viewDormitory"])){
                                    include_once("../controllers/room.php");
                                    $c_Room = new C_Room;
                                    $roomList = $c_Room->getRooms($_POST["id_dormitory"]);
                                }
                                if(isset($_POST["updateRoom"])){
                                    include_once("../controllers/room.php");
                                    $c_Room = new C_Room;
                                    $result = $c_Room->updateRoom($_POST["id_room"],$_POST["name_room"],$_POST["beds"],$_POST["price"],$_POST["note"]);
                                    $roomList = $c_Room->getRooms($_POST["id_dormitory"]);  
                                }
                                if(isset($_POST["deleteRoom"])){
                                    include_once("../controllers/room.php");
                                    $c_Room = new C_Room;
                                    $result = $c_Room->deleteRoom($_POST["id_room"]);
                                    $roomList = $c_Room->getRooms($_POST["id_dormitory"]);  
                                }
                                if ($roomList->num_rows > 0) {
                                    $i=0;
                                    while($row = $roomList -> fetch_assoc()) {
                            ?>
                        <tr id="i<?php echo $row["id_room"] ?>">
                            <form action="../views/room.php" method="post">
                            <td><?php echo $row["name_room"]; ?></td>
                            <td><?php echo $row["beds"]; ?></td>
                            <td><?php echo $row["price"]; ?></td>
                            <td><?php echo $row["note"]; ?></td>
                            <input style="display:none" type="text" name="id_dormitory" value="<?php echo $row["id_dormitory"] ?>">
                            <input style="display:none" type="text" name="id_room" value="<?php echo $row["id_room"] ?>">
                            <input style="display:none" type="text" name="name_room" value="<?php echo $row["name_room"] ?>">
                            <input style="display:none" type="text" name="name_dormitory" value="<?php echo $_POST["name_dormitory"] ?>">
                            <td>
                            <button class="btn" type="submit" name="deleteRoom">Xóa </button>
                            </form>
                            <button class="btn" onclick='openUpdate("<?php echo $row["id_room"] ?>")'>Sửa</button>
                            <form action="../views/registerR.php" method="post">
                            <input style="display:none" type="text" name="id_dormitory" value="<?php echo $row["id_dormitory"] ?>">
                            <input style="display:none" type="text" name="id_room" value="<?php echo $row["id_room"] ?>">
                            <input style="display:none" type="text" name="name_room" value="<?php echo $row["name_room"] ?>">
                            <input style="display:none" type="text" name="name_dormitory" value="<?php echo $_POST["name_dormitory"] ?>">
                            <input style="display:none" type="text" name="semester" value="20212">
                            <button class="btn" type="submit" name="viewRoomRegister">Xem</button>
                            </form>
                            </td>                    
                        </tr>
                        <tr style="display:none" id="<?php echo $row["id_room"] ?>">
                            <form action="../views/room.php" method="post">
                            <td><input type="text" name="name_room" value="<?php echo $row["name_room"]; ?>"></td>
                            <td><input type="text" name="beds" value="<?php echo $row["beds"]; ?>"></td>
                            <td><input type="text" name="price" value="<?php echo $row["price"]; ?>"></td>
                            <td><input style="width:100%" type="text" name="note" value="<?php echo $row["note"]; ?>"></td>
                            <td>
                            <button class="btn"  type="submit" name="updateRoom" value="<?php echo $row["id_room"] ?>">Lưu</button>
                            <input style="display:none" type="text" name="id_room" value="<?php echo $row["id_room"] ?>">
                            <input style="display:none" type="text" name="id_dormitory" value="<?php echo $row["id_dormitory"] ?>">
                            </form>
                            <button class="btn" onclick='closeUpdate("<?php echo $row["id_room"] ?>")'>Hủy</button> 
                            </td>
                        </tr>
                            <?php
                                        $i++;
                                    }
                                }
                                else{
                                    echo "No result found";
                                }
                            ?>

                    </table>
                </div>
            </div>
        </div>
        <script>
        function openUpdate(para){
            document.getElementById(para).style.display = "";
            document.getElementById("i"+para).style.display = "none";
        }
        function closeUpdate(para){
            document.getElementById(para).style.display = "none";
            document.getElementById("i"+para).style.display = "";
        }
    </script>
    </div>
    <?php include_once("footer.php") ?>
</body>
</html>